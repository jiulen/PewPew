...
void CSolidObject::Render(void)
{
	...
	glBindTexture(GL_TEXTURE_2D, iTextureID);
		...
		glBindVertexArray(VAO);
			if (mesh && !renderIcon)
				...
				if (renderIcon && iTextureID_Icon > 0)
				{
					glBindVertexArray(0);
					glBindVertexArray(VAO_ICON);
					glBindTexture(GL_TEXTURE_2D, 0);
					glBindTexture(GL_TEXTURE_2D, iTextureID_Icon);

					glDrawElements(GL_TRIANGLES, iIndicesSize_Icon, GL_UNSIGNED_INT, 0);
				}
				else
	...
	if (this->bIsDisplayed && !renderIcon)
	{
