 ...
 */
CTreeLobloly3D::CTreeLobloly3D(void)
 ...
 */
CTreeLobloly3D::CTreeLobloly3D(	const glm::vec3 vec3Position,
...
bool CTreeLobloly3D::Init(void)
{
	...
	std::string file_path = "Models/TreeLobloly/LoblolyPineTree.obj";
	...
	iTextureID = CImageLoader::GetInstance()->LoadTextureGetID("Models/TreeLobloly/LoblolyPineTree.tga", false);
	if (iTextureID == 0)
	{
		cout << "Unable to load Models/TreeLobloly/LoblolyPineTree.tga" << endl;
		return false;
	}
	...
	if (bInstancedRendering)
	{
		...
		srand((unsigned int)glfwGetTime() + 3);
		...
		for (unsigned int i = 0; i < iNumOfInstance; i++)
		{
			...
			while ((randX > 35 && randX < 65 && randZ > 35 && randZ < 65) ||
				   (randX > 15 && randX < 30 && randZ > 55 && randZ < 70)); //dont spawn in center area and house area

			float x = vec3Position.x + randX * fSpreadDistance * 0.01f - fSpreadDistance * 0.5f;
			float z = vec3Position.z + randZ * fSpreadDistance * 0.01f - fSpreadDistance * 0.5f;
			float y = CTerrain::GetInstance()->GetHeight(x, z);
			model = glm::translate(model, glm::vec3(x, y, z));

			// 2. Use scaling to create trees of various sizes
			float scale = (rand() % 50) * 0.0005f + 0.02f;
 ...
 */
void CTreeLobloly3D::SetModel(const glm::mat4 model)
{
	this->model = model;
}
 ...
 */
void CTreeLobloly3D::SetView(const glm::mat4 view)
{
	this->view = view;
}
 ...
 */
void CTreeLobloly3D::SetProjection(const glm::mat4 projection)
{
...
*/
void CTreeLobloly3D::SetInstancingMode(const bool bInstancedRendering)
{
 ...
 */
void CTreeLobloly3D::SetNumOfInstance(const unsigned int iNumOfInstance)
{
 ...
 */
void CTreeLobloly3D::SetSpreadDistance(const float fSpreadDistance)
{
 ...
 */
bool CTreeLobloly3D::Update(const double dElapsedTime)
{
	if (!bInstancedRendering)
	{
...
*/
void CTreeLobloly3D::PreRender(void)
{
...
*/
void CTreeLobloly3D::Render(void)
{
	if (bInstancedRendering)
	{
...
*/
void CTreeLobloly3D::PostRender(void)
{
...
/**
 @brief Print Self
 */
void CTreeLobloly3D::PrintSelf(void)
{
	cout << "CTreeLobloly3D::PrintSelf()" << endl;
	cout << "========================" << endl;
}
