...
bool CBush::Init(void)
{
	...
	std::string file_path = "Models/Bush/grass.obj";
	...
	iTextureID = CImageLoader::GetInstance()->LoadTextureGetID("Models/Bush/grass.tga", false);
	if (iTextureID == 0)
	{
	...
	if (bInstancedRendering)
	{
		...
		srand((unsigned int)glfwGetTime() + 6);
		...
		for (unsigned int i = 0; i < iNumOfInstance; i++)
		{
			...
			do
			{
				...
				randZ = rand() % 99;
				randZ++;
			} while (randX > 18 && randX < 25 && randZ > 58 && randZ < 67); //dont spawn in house area
 ...
 */
void CBush::SetModel(const glm::mat4 model)
{
	this->model = model;
}
 ...
 */
void CBush::SetView(const glm::mat4 view)
{
	this->view = view;
}
 ...
 */
void CBush::SetProjection(const glm::mat4 projection)
{
...
*/
void CBush::SetInstancingMode(const bool bInstancedRendering)
{
 ...
 */
void CBush::SetNumOfInstance(const unsigned int iNumOfInstance)
{
 ...
 */
void CBush::SetSpreadDistance(const float fSpreadDistance)
{
 ...
 */
bool CBush::Update(const double dElapsedTime)
{
	if (!bInstancedRendering)
	{
...
/**
 @brief Print Self
 */
void CBush::PrintSelf(void)
{
	cout << "CBush::PrintSelf()" << endl;
	cout << "========================" << endl;
}
