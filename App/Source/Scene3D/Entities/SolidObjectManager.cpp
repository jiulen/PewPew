...
bool CSolidObjectManager::CheckForCollision(void)
{
	...
	std::list<CSolidObject*>::reverse_iterator r_it, r_end;

	// Check for collisions between entities and projectiles
	r_end = lSolidObject.rend();
	for (r_it = lSolidObject.rbegin(); r_it != r_end; ++r_it)
	{
		// If the entity is not active, then skip it
		if ((*r_it)->GetStatus() == false)
			continue;
		...
		for (unsigned int i = 0; i < cProjectileManager->vProjectile.size(); i++)
		{
			...
			if (CCollisionManager::RayBoxCollision((*r_it)->GetPosition() + (*r_it)->boxMin,
				(*r_it)->GetPosition() + (*r_it)->boxMax,
				(cProjectileManager->vProjectile[i])->GetPreviousPosition(),
				(cProjectileManager->vProjectile[i])->GetPosition()))
			{
				if ((*r_it)->GetType() == CSolidObject::TYPE::PLAYER)
				{
					// If this projectile is fired by the player, then skip it
					if ((cProjectileManager->vProjectile[i])->GetSource() == (*r_it))
						continue;

					(*r_it)->health -= (cProjectileManager->vProjectile[i])->damage;
					(*r_it)->hurt = true;
				...
				else if ((*r_it)->GetType() == CSolidObject::TYPE::NPC)
				{
					...
					if ((*r_it)->health <= 0)
					{
						(*r_it)->SetStatus(false);
						++enemiesKilled;
					}
					else if ((*r_it)->fear == 2)
						(*r_it)->hurt = true;
					else if ((*r_it)->fear == 1 && (*r_it)->health <= 50)
						(*r_it)->hurt = true;
				...
				else if ((*r_it)->GetType() == CSolidObject::TYPE::STRUCTURE)
				{
					(cProjectileManager->vProjectile[i])->SetStatus(false);
			...
			// Check for collisions between the 2 entities
			if (CCollisionManager::BoxBoxCollision((*r_it)->GetPosition() + (*r_it)->boxMin,
				(*r_it)->GetPosition() + (*r_it)->boxMax,
