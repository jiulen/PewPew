									...
									vec3df vec3dfSoundPos)
{
	...
	if (eSoundType == CSoundInfo::SOUNDTYPE::_2D)
		cSoundInfo->Init(ID, pSoundSource, bIsLooped, bIsOverride);
	else
		cSoundInfo->Init(ID, pSoundSource, bIsLooped, bIsOverride, eSoundType, vec3dfSoundPos);
...
void CSoundController::PlaySoundByID(const int ID)
{
	...
	else if (cSoundEngine->isCurrentlyPlaying(pSoundInfo->GetSound()) && !pSoundInfo->GetOverrideStatus())
	{
