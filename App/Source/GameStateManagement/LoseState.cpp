...
bool CLoseState::Update(const double dElapsedTime)
{
	...
	// 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
	{
		...
		ImGui::Begin("Lose Menu", NULL, window_flags);
		ImGui::SetWindowPos(ImVec2(CSettings::GetInstance()->iWindowWidth/2.0 - buttonWidth/2.0, 
			CSettings::GetInstance()->iWindowHeight/5.0 * 1.5));				// Set the top-left of the window at (10,10)
		ImGui::SetWindowSize(ImVec2(CSettings::GetInstance()->iWindowWidth, CSettings::GetInstance()->iWindowHeight));

		//Added rounding for nicer effect
		ImGuiStyle& style = ImGui::GetStyle();
		style.FrameRounding = 200.0f;
		style.ItemSpacing = ImVec2(0, 100);

		ImGui::Image((void*)(intptr_t)CImageLoader::GetInstance()->LoadTextureGetID("Image/GUI/lose.tga", true),
			ImVec2(256, 64),
			...
			ImVec2(buttonWidth, buttonHeight), ImVec2(0.0, 0.0), ImVec2(1.0, 1.0)))
		{
			...
			// Load the menu state
			cout << "Quitting the game from LoseState" << endl;
